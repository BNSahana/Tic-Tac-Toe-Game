...
function PlayArea({
    ...
    useEffect(() => {
        if (turn !== player) {
            ...
            setTimeout(() => {
                if (blockIndex < 0) {
                    markInput(findBestChoice());
                    ticTacRef.current.style.pointerEvents = "auto";
                } else {
                    markInput(blockIndex);
                }
            }, 1000);
        }
    }, [turn]);
    ...
    useEffect(() => {
        if (markedIndexes.length >= 5) {
            ...
            if (isWin || markedIndexes.length === 9) {
                gameOver(isWin);
            }
        }
    ...
    const gameOver = (isWin) => {
        markedIndexes = [];
        if (turn !== player) {
            ticTacRef.current.style.pointerEvents = "none";
        }
        let gameOverProps = {};
        setTimeout(() => {
            if (isWin) {
    ...
    const findBlockChoice = useCallback((index, player) => {
        ...
        if ((rowCount === 2 && rowCpuIndex !== -1) && (colCount === 2 && colCpuIndex !== -1)) {
            return Math.max(rowCpuIndex, colCpuIndex);
        } else if (rowCount === 2 && rowCpuIndex !== -1) {
            return rowCpuIndex;
        } else if (colCount === 2 && colCpuIndex !== -1) {
            return colCpuIndex;
        }
        ...
        if (index % 2 === 0) {
            ...
            if (diagonalCount === 2 && diagIndex !== -1) {
                return diagIndex;
            } else {
